...
void Client::mainLoop() {
	while (true) {
		...
		while (this->connected_to_an_account) {
			...
			if (output == "GAME" or parser.getQueryType() == QUERY_TYPE::CREATE_GAME) { this->gameLoop(); continue;}
...
std::string Client::analyseServerResponse(std::string &output) {
    ...
    // Fin d'une partie
    else if ( output == "ENDGAME" ) { this->in_game = false; return ""; }
    // Autre
    else { return output; }
}
...
void Client::receiveFromServerLoop() {
	while (this->in_game) {
		std::string output;
		this->receiveFromServer(output); // get the output from the server
        std::cout << this->analyseServerResponse(output); // Analyse the output from the server
        // If output is "ENDGAME" it should stop the loop.
	}
}
